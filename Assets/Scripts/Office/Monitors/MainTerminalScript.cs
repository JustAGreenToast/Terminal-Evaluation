...
public class MainTerminalScript : MonitorWindow
{
    ...
    public Exercise currentExercise { get { return exercises.Count > 0 ? exercises.First.Value : null; } }
    int solvedExercises;
    int exercisesLeft;
    ...
    {
        get { return gitScripts.ContainsKey(currentExercise.folderName) ? gitScripts[currentExercise.folderName] : null; }
        set
        {
            if (!gitScripts.ContainsKey(currentExercise.folderName))
            {
                gitScripts.Add(currentExercise.folderName, value);
                if (SettingsManager.settings.deluxeStatusPanel) { deluxeStatusPanel.GitScriptAdded(); }
            }
            else { gitScripts[currentExercise.folderName] = value; }
        }
    }
    ...
    protected override void OnStart()
    {
        ...
        if (SettingsManager.settings.deluxeStatusPanel) { deluxeStatusPanel.InitRoadmap(exercises, exercisesLeft); }
        ...
        if (currentStatus == Status.Evaluating)
        {
            if (evalTask.IsCompleted)
            {
                if (evalTask.Result == MoulinetteTerminal.Status.OK)
                {
                    ...
                    if (SettingsManager.settings.deluxeStatusPanel) { deluxeStatusPanel.ExerciseCleared(); }
                ...
                else
                {
                    ...
                    if (SettingsManager.settings.deluxeStatusPanel) { deluxeStatusPanel.ExerciseFailed(); }
                }
            }
        }
    }
        ...
        switch (args[0].ToLower())
        {
            ...
            case "startexam":
                ...
                else
                {
                    ...
                    manager.ExamStarted();
                    manager.ExerciseStarted();
                    if (SettingsManager.settings.deluxeStatusPanel) { deluxeStatusPanel.ExerciseStarted(); }
            ...
            case "status":
                ...
                // Display Status
                else { if (SettingsManager.settings.deluxeStatusPanel) { shellOutput.text = ""; deluxeStatusPanel.Show(); } else { SetOutput(GetStatusOutput()); } }
                break;
            ...
            case "mkcd":
                ...
                // Folder Name Check
                else if (args[1] != exercises.First.Value.folderName) { SetOutput("Are you sure that's the correct folder name?"); }
                // Folder Exists Check
            ...
            case "nano":
                ...
                // File Name Check
                else if (args[1] != exercises.First.Value.fileName) { SetOutput("Are you sure that's the correct file name?"); }
            ...
            case "mcv":
                ...
                // Name Check
                else if (args[1] != exercises.First.Value.folderName && args[1] != exercises.First.Value.fileName) { SetOutput("Are you sure that's the correct name?"); }
    ...
    void StartLap2()
    {
        ...
        currentStatus = Status.ExamStarted;
        manager.Lap2Started();
        if (SettingsManager.settings.deluxeStatusPanel) { deluxeStatusPanel.Lap2Started(); }
        RunCommand("status", true);
    }
    string GetStatusOutput()
    {
        ...
        Exercise ex = exercises.First.Value;
    ...
    {
        if (SettingsManager.settings.deluxeStatusPanel) { deluxeStatusPanel.Hide(); }
